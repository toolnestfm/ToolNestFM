import { apiErr } from '@/lib/api-response';
import { geminiCompleteServer } from '@/lib/gemini/server';
import { readJson, requireApiKey, withCredits } from '@/lib/api-v1';

export const dynamic = 'force-dynamic';

/**
 * POST /api/v1/translate — translate text. 1 credit.
 * Body: { "text": "...", "to": "Hindi", "from"?: "auto" }
 */
export async function POST(req: Request) {
  const gate = await requireApiKey(req, 30);
  if (!gate.ok) return gate.response;

  const body = await readJson<{ text?: string; to?: string; from?: string }>(req);
  if (!body) return apiErr('Invalid JSON body', 400);

  const text = body.text?.trim();
  const to = body.to?.trim().slice(0, 40);
  if (!text) return apiErr('text is required', 400);
  if (!to) return apiErr('to (target language) is required, e.g. "Hindi"', 400);
  if (text.length > 100_000) return apiErr('text too large (max 100,000 characters)', 400);
  if (!process.env.GEMINI_API_KEY) return apiErr('AI backend is not configured', 503);

  const from = body.from?.trim().slice(0, 40);
  const system = `You are a professional translator. Translate the user's text ${from ? `from ${from} ` : ''}to ${to}. Preserve tone and formatting. Return only the translation — no preamble.`;

  return withCredits(gate.ctx, 1, async () => {
    const translation = await geminiCompleteServer([{ role: 'user', content: text }], system);
    return { translation, to };
  });
}
